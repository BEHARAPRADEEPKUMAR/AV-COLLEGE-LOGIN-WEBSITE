...
def signup(request):
    if request.method == "POST":
        ...
        fname = request.POST.get('fname', '')  # Default to empty string if key is missing
        lname = request.POST.get('lname', '')  # Default to empty string if key is missing
        ...
        if User.objects.filter(rollno=rollno):
            messages.error(request, "Username already exist! Please try some other rollno.")
        ...
        if not rollno.isalnum():
            messages.error(request, "Username must be a Number Only!!")
            return redirect('signup')
...
def profile(request, rollno):
    user = get_object_or_404(User, rollno=rollno)  # Get user by roll number
